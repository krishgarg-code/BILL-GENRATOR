...
const BillGenerator2 = ({ onLogout }) => {
    ...
    const handleBillsPerPageChange = (count) => {
        ...
        setBills(prev => {
            let newBills = [...prev];
            if (newBills.length < newCount) {
                // Add new bills
                const billsToAdd = newCount - newBills.length;
                for (let i = 0; i < billsToAdd; i++) {
                    newBills.push({ id: Date.now() + i, formData: { ...initialFormData }, items: [] });
                }
            } else if (newBills.length > newCount) {
                // Remove excess bills
                newBills = newBills.slice(0, newCount);
            }
            return newBills;
        });
        ...
        if (currentBillIndex >= newCount) {
            setCurrentBillIndex(newCount - 1);
        }
    };
                    ...
                    <TabsList className="bg-zinc-100 dark:bg-zinc-800 mb-4 w-full justify-start overflow-x-auto h-auto min-h-10 py-1">
                        {bills.map((bill, index) => (
                            <TabsTrigger
                                ...
                                className="px-4 py-2 data-[state=active]:bg-[#00963C] data-[state=active]:text-white whitespace-nowrap"
                            >
                                Bill {index + 1}
